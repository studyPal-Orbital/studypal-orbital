...
describe('User can type out tasks on a sticky note', () => {
    ...
    it('User profile analytics are provided', () => {
        cy.get('[data-cy="profile"]').contains('Focus Sessions')
        cy.get('[data-cy="profile"]').contains('Task Completion')
        cy.get('[data-cy="profile"]').contains('tasks completed')
        cy.get('[data-cy="profile"]').contains('hours spent studying')
    })
})
